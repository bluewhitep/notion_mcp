...
- 真实调用前，需要先用 `notion-mcp config --global user.token <token>` 设置全局 token，并确保 Notion connection 已被授权访问目标页面、数据库或 workspace 内容。
...
notion-mcp block trash <block_id> --dry-run
```
