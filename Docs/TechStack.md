...
MCP Python SDK 是实现真正 MCP server 的依赖。
...
- legacy tests：保护旧 FastAPI REST 和旧 CLI 行为。
- Core tests：覆盖配置、client factory、service 和错误模型。
- CLI tests：覆盖命令注册、输出格式、dry-run 和 Core 调用边界。
- MCP tests：覆盖 server 生命周期、tool inventory、tool call 和危险操作确认。
- scenario tests：覆盖隔离安装和端到端 mock workflow。
- live tests：访问真实 Notion，默认跳过。
