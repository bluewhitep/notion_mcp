...
from notion_mcp.core.project import ProjectPaths, ProjectResolver
...
# Usage:
# notion-mcp database attach DATABASE_ID --data-source Tasks --json
# --------------------------------
def attach_database(
    database_id: str,
...
def database_status(
    ...
    try:
        ...
        if refresh:
            ...
            attachment = attachment.refreshed(database_response, command="notion-mcp database status --refresh")
            DatabaseAttachmentStore.save(project_root, attachment)
    except CoreError as exc:
        exit_with_error(exc, json_output=json_output)
    if json_output:
...
# Usage:
# notion-mcp database refresh --json
# --------------------------------
def database_refresh(
    ...
    try:
        project_root, attachment = load_current_database_attachment()
        database_response = get_databases_service().retrieve(attachment.database.id)
        updated = attachment.refreshed(database_response, command="notion-mcp database refresh")
...
# Usage:
# notion-mcp database sources DATABASE_ID --json
# --------------------------------
def sources(
    database_id: str | None = typer.Argument(None),
    json_output: bool = typer.Option(False, "--json", help="Print JSON output"),
...
# Usage:
# notion-mcp database create --payload '{"parent": {}}' --json
# --------------------------------
def create(
...
# Usage:
# notion-mcp database update DATABASE_ID --payload '{"title": []}' --json
# --------------------------------
def update(
    database_id: str,
...
# Usage:
# notion-mcp database rename DATABASE_ID "New Name" --json
# --------------------------------
def rename(
    database_id: str,
    new_name: str,
